function [HM_CO2, NRMSE_CO2, HM_CH4,NRMSE_CH4,BubbleFlux,Max_Jump,r2_CH4,r2_CO2]=ProcessLI7810Chamber(meas_date,start_time,end_time,L_data_sheet,Start_offset,End_offset,ChamberVolume,ChamberArea, plotYN,Site,Start_temp,End_temp,Pressure,Offset,~)
...
R=.000082057;                  %m3 atm K-1 mol-1
CO2=CO2*Pressure/R./TMP;  %umol/m^3
CH4=CH4*Pressure/R./TMP/1000;  %umol/m^3

%% Trim out any data at beginning or end of measurement period that were affected by chamber settling
[~,IndSt]=min(abs(TIME-Start_offset));
[~,IndEn]=min(abs(TIME-(TIME(end)-End_offset))); 

T_TIME=TIME(IndSt:IndEn);
T_H2O =H2O (IndSt:IndEn);
T_CO2 =CO2 (IndSt:IndEn);
T_CH4 =CH4 (IndSt:IndEn); 

if sum(~isnan(T_CH4)) < 50 %if you only had 50 points for a sample, don't count it
    %% Too few points to run regression on
    warning('Too few points to run regression on');
    HM_CH4 = nan;
    HM_CO2=nan;
    BubbleFlux=nan;
    NRMSE_CH4= nan;
    ...
    BubThr = 1.4914; %August 2022 - TIM: This should probably be passed in as a user input/ (ppm/s)
    ...
    T_CH4nb = [T_CH4(1); T_CH4(2:end) - TotBubble]; %new matrix; T_CH4 start and stop; starts at 1 and goes from 2:end.
                                                            %creates a matrix of methane exluding CH4 from bubbling events= total diffusive flux
    BubbleFlux = (TotBubble(end)/(T_TIME(end) - T_TIME(1)))*ChamberVolume/ChamberArea; % umol m-2 s-1
    ...
    nn=max([5,find(~isnan(T_CH4_rm),1,'first')]);
    Start_co_CH4 = mean(T_CH4_rm(1:nn),'omitnan'); % Initial concentration value
    ...
    try
        x=nlinfit(T_TIME(~isnan(T_CH4_rm)),T_CH4_rm(~isnan(T_CH4_rm)),@myfun2,x0,opts);
    catch
        try
            x0(3)=x0(3)*-1;
            x=nlinfit(T_TIME(~isnan(T_CH4_rm)),T_CH4_rm(~isnan(T_CH4_rm)),@myfun2,x0,opts);
...
%     x = lsqcurvefit(@myfun2,x0,T_TIME(~isnan(T_CH4_rm)),T_CH4_rm(~isnan(T_CH4_rm))); %Apply curve fit
    ...
    % Initial conditions for HM parameters
    nn=max([5,find(~isnan(T_CO2_rm),1,'first')]);
    Start_co_CO2 = mean(T_CO2_rm(1:nn),'omitnan'); % Initial concentration value
    Start_cs_CO2 = 100;                             % Saturation concentration value
    Start_k_CO2 = 0.1;                              % First order-rate constant
    z0=[Start_cs_CO2,Start_co_CO2,Start_k_CO2];
    
    try
        z=nlinfit(T_TIME(~isnan(T_CO2_rm)),T_CO2_rm(~isnan(T_CO2_rm)),@myfun3,z0,opts);
    catch
        try
            z0(3)=z0(3)*-1;
            z=nlinfit(T_TIME(~isnan(T_CO2_rm)),T_CO2_rm(~isnan(T_CO2_rm)),@myfun3,z0,opts);
...
%     z = lsqcurvefit(@myfun3,z0,T_TIME(~isnan(T_CO2_rm)),T_CO2_rm(~isnan(T_CO2_rm)));
    ...
    if plotYN==1
        ...
        [~,~,~,~,stats]=regress(y,[T_CH4_rm ones(length(T_CH4_rm),1)]);
        r2_CH4=stats(1);
        ...
        %CO2 RMSE
        [~,~,~,~,stats]=regress(y2,[T_CO2_rm ones(length(T_CO2_rm),1)]);
        r2_CO2=stats(1);
        ...
        plot(TIME,CH4,'-g');
        plot(T_TIME,T_CH4,'-b')
        plot(T_TIME,T_CH4nb,'-c');
        plot(T_TIME,T_CH4_rm,'-k');
        ylabel('[CH_4] (\mumol m^{-3})');
        title(Site);
        
        yyaxis right
        plot(T_TIME(2:end),TotBubble,'-r');
        ylabel('Bubbled [CH_4] (\mumol m^{-3})');
        %legend('Shaved','Raw','Despiked','Final','Bubbles');
        
        subplot(3,1,2);
        %CH4 plots without outliers
        hold on;
        plot(T_TIME,y,'-r','MarkerSize',15); %Curve fit with HM line
